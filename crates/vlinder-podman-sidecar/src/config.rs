...
impl SidecarConfig {
    ...
    ///
    /// Required: `VLINDER_AGENT`, `VLINDER_QUEUE_BACKEND`, `VLINDER_REGISTRY_URL`,
    /// `VLINDER_STATE_URL`.
    /// NATS backend requires: `VLINDER_NATS_URL`.
    /// SQS backend requires: `VLINDER_SQS_REGION`, `VLINDER_SQS_QUEUE_PREFIX`.
    pub fn from_env() -> Result<Self, String> {
        ...
        let queue = match backend.as_str() {
            ...
            #[cfg(not(feature = "sqs"))]
            "sqs" => return Err("SQS support not compiled — enable the 'sqs' feature".into()),
