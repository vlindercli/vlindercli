...
pub(crate) trait LambdaClient: Send + Sync {
    ...
    async fn create_function(&self, req: &CreateFunctionRequest<'_>)
        -> Result<String, LambdaError>;

    /// Get info about a Lambda function, or None if it doesn't exist.
    async fn get_function(&self, function_name: &str) -> Result<Option<FunctionInfo>, LambdaError>;
...
impl AwsLambdaClient {
    /// Create a new AWS client for the given region (async, no nested runtime).
    ///
    /// Uses the default credential chain (~/.aws/credentials, env vars, etc).
    pub(crate) async fn new_async(region: &str) -> Result<Self, LambdaError> {
...
impl LambdaClient for AwsLambdaClient {
    ...
    async fn create_role(&self, role_name: &str) -> Result<String, LambdaError> {
        ...
        self.iam
            ...
            .send()
            .await
            .map_err(|e| LambdaError::Aws(format!("put_role_policy: {}", format_sdk_error(&e))))?;
    ...
    async fn get_function(&self, function_name: &str) -> Result<Option<FunctionInfo>, LambdaError> {
        self.get_function_inner(function_name).await
    }
